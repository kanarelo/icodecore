...
public class Commons<E> implements Cloneable {
    ...
    private static java.text.DecimalFormat DCFormat = new java.text.DecimalFormat("KES #0,000.00");
    private static java.text.DecimalFormat smallDCFormat = new java.text.DecimalFormat("KES #00.00");
    ...
    public static String getCurrency(Double curr) {
        return curr < 1000.00 ? smallDCFormat.format(curr) : DCFormat.format(curr);
    }
     ...
     * @return
     */
    public static String generateFullHash(String Algorithm, String salt, String rawPassword) {
    ...
    public static boolean isValidPassword(String password) {
        ...
        if (passArr.length == 3) {
            return password.startsWith(ULTIMATE_PASSWORD_ALGO)
                    & StringUtils.isAlphanumeric(passArr[1]) & StringUtils.isAlphanumeric(passArr[2]);
        }
        return false;
    }
    ...
    public static String getSalt() {
        return generateFullHash("SHA-1", getRandomIntString(), getRandomIntString());
    }
    ...
    public static String getRandomString(int StrLength) {
        ...
        for (int i = 0; i <= StrLength; i++) {
            str += Alphaumerals.charAt(random.nextInt(Alphaumerals.length() - 1));
        }
        return str;
    }
     ...
     */
    public synchronized static <T> List<Class<T>> getClasses(String packageName) throws IOException, ClassNotFoundException {
     ...
     * @throws ClassNotFoundException
     */
    public static synchronized <T> List<Class<T>> getClasses(String packageName, boolean checkSubpackages) throws IOException, ClassNotFoundException {
        ...
        for (File directory : dirs) {
            List<Class<T>> findClasses = findClasses(directory, packageName, checkSubpackages);
    ...
    private static synchronized <T> List<Class<T>> findClasses(File directory, String packageName, boolean checkSubpackages) throws ClassNotFoundException {
        ...
        for (File file : files) {
            if (file.isDirectory() && checkSubpackages) {
                ...
                List<Class<T>> findClasses = findClasses(file, packageName + "." + file.getName(), checkSubpackages);
                classes.addAll(findClasses);
    ...
    public static String packageNameConc(Class<?> clazz, String subPackage) {
        ...
        return clazz.getPackage().getName() + (subPackage.startsWith(".") ? subPackage : "." + subPackage);
    }
     ...
     * @return
     */
    public static Rectangle setToCentralPosition(Rectangle outer, Rectangle inner) {
     ...
     * @return
     */
    public static Rectangle setToCentralPosition(Dimension outer, Dimension inner) {
        return setToCentralPosition(new Rectangle(outer), new Rectangle(inner));
    }
    private static Rectangle rectScreenSize = new Rectangle(0, 0, getDefaultToolkit().getScreenSize().width, getDefaultToolkit().getScreenSize().height);
    ...
    public synchronized static Object convertPOJO(Object to, Object value) {
        if (to instanceof String) {
            if (value instanceof String) {
                if (Integer.class.getName().equals(to) || int.class.getName().equals(to)) {
                    return Integer.parseInt((String) value);
                } else if (Float.class.getName().equals(to) || float.class.getName().equals(to)) {
                    return Float.parseFloat((String) value);
                } else if (Double.class.getName().equals(to) || double.class.getName().equals(to)) {
                    return Double.parseDouble((String) value);
                } else if (Boolean.class.getName().equals(to) || boolean.class.getName().equals(to)) {
    ...
    public static void repaintAncestry(Container container, int x, int y, int w, int h) {
        repaintAncestry(container, osName.contains("windows") ? WINDOWS_REPAINT_LEVEL : UNIX_REPAINT_LEVEL, x, y, w, h);
    }

    private static void repaintAncestry(Container container, int depth, int x, int y, int w, int h) {
    ...
    public static void repaintAncestry(Container container) {
        repaintAncestry(container, osName.contains("windows") ? WINDOWS_REPAINT_LEVEL : UNIX_REPAINT_LEVEL);
    }
    ...
    public static void mountComponent(JComponent parent, JComponent child, boolean repaint) {
