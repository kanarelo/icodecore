...
class BasePropertySet {
	...
	public static final Logger LOG = ICodeUtils
			.getLogger(BasePropertySet.class);
	 ...
	 * @throws IOException
	 *             If there was a problem loading the properties from the
	 *             supplied {@link InputStream}.
	 */
	static void load(Map<String, String> map, InputStream inputStream)
			throws IOException {
		if (inputStream == null)
			throw new NullPointerException(
					"The supplied input stream was null.");
		...
		try {
			...
			while ((line = in.readLine()) != null) {
				...
				if (line.length() == 0 || line.charAt(0) == '#') {
					// This is a comment, so we should remember it to write back
					// later
				} else {
					...
					if (splitChar <= 0) {
						...
						// not strictly legal, so we'll log a warning and carry
						// on.
	...
	static HashMap<String, String> load(InputStream inputStream)
			throws IOException {
